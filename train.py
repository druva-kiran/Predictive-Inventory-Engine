...
# ── dtype guard — call this before saving ANY dataframe to the checkpoint ─────
def enforce_float32(df: pd.DataFrame) -> pd.DataFrame:
    """
    Coerce every column to float32 and fill NaN with 0.

    Root causes this prevents:
    1. idx.isocalendar().week.values returns a UInt32 Series that pandas
       silently stores as dtype=object in some versions — poisons week_sin/cos.
    2. df_oil dcoilwtico has NaN gaps; after ffill() edge-rows can be NaN
       objects that survive as dtype=object when concatenated.
    3. Rolling/lag operations on a mixed-type frame can produce object columns.

    Calling this once before torch.save() guarantees the checkpoint is clean
    and the MinMaxScaler in app.py never receives non-numeric data.
    """
    for col in df.columns:
        if df[col].dtype == object or str(df[col].dtype).startswith("UInt"):
...
def build_features(df_train, df_oil, df_holidays, store_nbr):
    ...
    # FIX: isocalendar().week returns UInt32 / object dtype in many pandas
    # versions. Cast explicitly to plain Python float via .astype(int) first.
    week_num = idx.isocalendar().week.astype(int).values
    df["week_sin"] = np.sin(2 * np.pi * week_num / 52).astype(np.float32)
    df["week_cos"] = np.cos(2 * np.pi * week_num / 52).astype(np.float32)
    ...
    # FIX: merge oil, then immediately coerce dcoilwtico to float.
    # ffill() + fillna(0) still leaves dtype=object if the column was
    # read as mixed-type from the CSV. Explicit cast eliminates this.
    df = pd.merge(df, df_oil.set_index("date"),
                  left_index=True, right_index=True, how="left").ffill().fillna(0)
    if "dcoilwtico" in df.columns:
        df["dcoilwtico"] = pd.to_numeric(df["dcoilwtico"], errors="coerce").fillna(0).astype(np.float32)

    df = df.iloc[28:].copy()

    # Final safety net: enforce clean float32 on the whole frame before returning
    df = enforce_float32(df)
...
# Verify no object columns made it through — fail loudly at train time,
# not silently at inference time on Render.
bad_cols = [c for c in df_feat.columns if df_feat[c].dtype == object]
if bad_cols:
    raise RuntimeError(
        f"[train] FATAL: dtype=object columns still present after enforce_float32: {bad_cols}\n"
        "Fix build_features() before training."
    )
print(f"[train] df_feat dtype check passed — all {len(df_feat.columns)} columns are numeric.")
...
if nearzero_cats:
    ...
    for cat in nearzero_cats:
        ...
        # FIX: coerce dcoilwtico in cross_df too — same dtype=object risk
        if "dcoilwtico" in cross_df.columns:
            cross_df["dcoilwtico"] = pd.to_numeric(
                cross_df["dcoilwtico"], errors="coerce"
        ...
        # FIX: enforce float32 on the cross_df before saving to checkpoint
        cross_df = enforce_float32(cross_df)
...
# Final dtype audit — guarantees the checkpoint is 100% clean float32.
# If any object column somehow survived, this will catch it here at train
# time with a clear error rather than a mysterious 500 on Render at 9 PM.
final_bad = [c for c in df_feat.columns if df_feat[c].dtype == object]
if final_bad:
    raise RuntimeError(f"[train] FATAL: object columns in df_feat at save time: {final_bad}")
for cat, cdf in cross_store_dfs.items():
    bad = [c for c in cdf.columns if cdf[c].dtype == object]
    if bad:
        raise RuntimeError(f"[train] FATAL: object columns in cross_store_dfs[{cat}]: {bad}")
...
print(f"[train] df_feat shape : {df_feat.shape}  (embedded, no CSV needed on Render)")
print(f"[train] df_feat dtypes: {df_feat.dtypes.value_counts().to_dict()}")
print("[train] Done! Push the .pth file to GitHub — no CSV needed on Render.")